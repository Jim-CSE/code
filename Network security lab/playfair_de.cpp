...
int main()
{
    ...
    for (int l = 0; l < ciphertext.length(); l += 2) {
        ...
        // Decryption logic based on Playfair cipher rules
        // If both letters are in the same row
        if (x1 == x2) {
            decryptedText += p[x1][(y1 - 1 + 5) % 5];  // Shift left for decryption
            decryptedText += p[x2][(y2 - 1 + 5) % 5];  // Shift left for decryption
        }
        // If both letters are in the same column
        else if (y1 == y2) {
            decryptedText += p[(x1 - 1 + 5) % 5][y1];  // Shift up for decryption
            decryptedText += p[(x2 - 1 + 5) % 5][y2];  // Shift up for decryption
        }
        // If the letters form a rectangle
        else {
